...
const canSeeFinancial = () => can("admin", "seller", "coordinator");
...
function receptionDetailHtml(s, compact = false) {
  const hasInfo = s.receptionIssueType || s.receptionNotes || s.receptionPhoto;
  if (!hasInfo) return "";
  const parts = [];
  if (s.receptionIssueType) parts.push(`<div class="reception-chip">${escapeHtml(s.receptionIssueType)}</div>`);
  if (s.receptionNotes) parts.push(`<div class="muted ${compact ? "tiny" : ""}">${escapeHtml(s.receptionNotes)}</div>`);
  if (s.receptionPhoto) parts.push(`<button class="secondary small" onclick="openPhotoModal('${s.id}')">Ver foto</button>`);
  ...
  app.innerHTML = `
    <div class="login">
      <div class="login-card">
        ...
        <div class="demo"><strong>Usuarios demo</strong><br>admin / admin123<br>antonio / 1234<br>coord / 1234<br>recepcao / 1234<br>operario / 1234<br>consulta / 1234</div>
...
function renderTopbar() {
  ...
  const newSaleVisible = can("admin", "seller", "coordinator") && ["agenda", "sales", "dashboard"].includes(state.view);
  return `
    ...
    <div class="layout">
      <aside class="sidebar">
        ...
        <nav class="menu">
          ...
          ${canSeeFinancial() ? menuButton("sales", can("seller") ? "Mis ventas" : "Ventas") : ""}
...
function renderCurrentView() {
  const map = { dashboard: renderDashboard, agenda: renderAgenda, sales: renderSales, reception: renderReception, production: renderProduction, delivery: renderDelivery, config: renderConfig };
  ...
  return `
    ...
    <section class="grid-2">
      <div class="card main-card">
        <div class="panel-title"><h2>Calendario del mes</h2><button class="secondary small" onclick="state.view='agenda';renderApp()">Abrir agenda</button></div>
    ...
    <section class="grid-2">
      ...
      <div class="card main-card">
        ...
        <div class="commission-box">
          ...
          <div class="commission-line"><span>Total comisión</span><strong>${money(c.totalCommission)}</strong></div>
          ...
          <div class="commission-line"><span>Tasa promedio</span><strong>${state.user.role === "seller" ? `${Number(state.user.commissionRate || state.config.commissionRate || 7)}%` : `${Number(state.config.commissionRate || 7)}% default`}</strong></div>
          <div class="muted">${can("seller") ? "Solo ves y editas tus propias ventas." : "El administrador puede ajustar la comisión individual de cada vendedor."}</div>
        </div>
      </div>
    </section>`;
}
...
function renderReception() {
  ...
  return `<div class="card main-card"><div class="panel-title"><h2>Recepción del día</h2></div><div class="note">Al marcar <strong>Recibido</strong> ahora puedes dejar observaciones del estado del auto y subir una foto del detalle.</div>${tableTemplate(rows)}</div>`;
}
function renderProduction() {
  const rows = state.sales.filter((s) => ["received", "in_progress"].includes(s.autoStatus));
  return `<div class="card main-card"><div class="panel-title"><h2>Producción</h2></div>${tableTemplate(rows)}</div>`;
}
function renderDelivery() {
  const rows = state.sales.filter((s) => ["ready_delivery", "delivered"].includes(s.autoStatus));
  return `<div class="card main-card"><div class="panel-title"><h2>Entrega</h2></div>${tableTemplate(rows)}</div>`;
}
  ...
  return `
    <section class="grid-2 config-grid">
      ...
      <div class="card main-card">
        ...
        <div class="note">Desde acá el administrador puede agregar o eliminar vendedores, definir su comisión y dejar accesos activos o inactivos.</div>
        ${userTable()}
      </div>
    </section>`;
}
...
function userTable() {
  ...
  return `
    <div class="table-wrap"><table>
      <thead><tr><th>Nombre</th><th>Usuario</th><th>Rol</th><th>Comisión</th><th>Estado</th><th>Acciones</th></tr></thead>
      <tbody>
        ${rows.map((u) => `
          <tr>
...
function tableTemplate(rows) {
  const financialCols = canSeeFinancial() ? `<th>Producto</th><th>Valor</th><th>Comisión</th>` : "";
  const cols = canSeeFinancial() ? 9 : 6;
...
function rowTemplate(s) {
  const financialCols = canSeeFinancial() ? `<td>${escapeHtml(s.product || "-")}</td><td>${money(s.saleValue)}</td><td>${Number(s.commissionRate || state.config.commissionRate || 7)}%</td>` : "";
  return `
    <tr>
...
window.openSaleModal = (id = null) => {
  ...
  const sale = editing
    ...
    : {
        ...
        client: "", phone: "", car: "", plate: "", product: "", saleValue: "", deposit: "", paymentMethod: "", paymentStatus: "PENDIENTE", origin: "", saleDate: todayISO(), entryDate: todayISO(), exitDate: plusDays(todayISO(), Number(state.config.defaultStayDays || 5)), entryTime: "09:00", notes: "", commissionRate: state.config.commissionRate || 7,
      };
  const root = document.getElementById("modalRoot");
  root.classList.add("open");
  ...
  root.innerHTML = `
    <div class="modal">
      ...
      <form id="saleForm" class="form-grid">
        ...
        ${canSeeFinancial() ? `<div><label>Valor de venta</label><input type="number" step="0.01" name="saleValue" value="${sale.saleValue || ""}" /></div><div><label>Seña</label><input type="number" step="0.01" name="deposit" value="${sale.deposit || ""}" /></div><div><label>% comisión</label><input type="number" step="0.1" name="commissionRate" value="${sale.commissionRate || sellerUser?.commissionRate || state.config.commissionRate || 7}" ${canEditFinancial() ? "" : "readonly"} /></div><div><label>Forma de pago</label><input name="paymentMethod" value="${escapeHtml(sale.paymentMethod || "")}" /></div><div><label>Estado del pago</label><select name="paymentStatus">${["PENDIENTE","PARCIAL","PAGO FINALIZADO"].map((x) => `<option ${sale.paymentStatus === x ? "selected" : ""}>${x}</option>`).join("")}</select></div>` : ""}
  ...
  document.querySelector('[name="seller"]')?.addEventListener("change", (e) => {
    ...
    if (seller && rateInput && !editing) rateInput.value = seller.commissionRate || state.config.commissionRate || 7;
  });
...
window.openUserModal = (id = null) => {
  const user = id ? state.users.find((u) => u.id === id) : { role: "seller", active: true, commissionRate: state.config.commissionRate || 7 };
  const editing = !!id;
  const root = document.getElementById("modalRoot");
  root.classList.add("open");
  root.innerHTML = `
  ...
  const syncCommissionField = () => {
    commissionInput.disabled = roleSelect.value !== "seller";
    commissionInput.closest("div").style.opacity = roleSelect.value === "seller" ? "1" : ".55";
  };
...
async function saveConfig(e) {
  e.preventDefault();
  const data = Object.fromEntries(new FormData(e.target).entries());
  ["weeklySlots", "dailyCapacity", "defaultStayDays", "commissionRate"].forEach((k) => data[k] = Number(data[k] || 0));
  try {
